...
export default function LoginProfessor() {
	...
	const sendData = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			await loginTeacher(formDataObj);
			router.push('/area-restrita/professor');
		} catch (error: any) {
			setLoading(false)
	...
	const forgotPsw = async () => {
		...
		try {
			await recoverTeacherPassword(email);
			setLoading(false);
			Swal.fire({
				title: 'Sucesso!',
			...
			<div className="h-screen bg-gradient-to-br from-[#f5f7fa] to-[#eef2f7] flex items-center justify-center px-4 py-6 overflow-hidden">
				<div className="w-full max-w-sm max-h-[calc(100vh-3rem)]">
					...
					<div className="bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col h-fit max-h-[calc(100vh-3rem)]">
						{/* Header com Gradient */}
						<div className="bg-gradient-to-r from-orange-400 to-orange-600 px-6 sm:px-8 py-6 sm:py-8 text-center shrink-0">
							...
							<p className="text-blue-100 text-sm">Acesso para professores</p>
						</div>
						...
						<div className="px-6 sm:px-8 py-6 sm:py-8 overflow-y-auto flex-1">
							<form id="login-form" onSubmit={sendData} className="space-y-5">
								...
								<button
									type="submit"
									className="w-full bg-gradient-to-r from-orange-400 to-orange-600 hover:from-orange-500 hover:to-orange-700 text-white font-semibold py-3 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 mt-8 cursor-pointer active:brightness-90"
								>
							...
							<Link
								...
								className="block w-full py-3 px-4 border-2 border-orange-500 text-orange-500 font-semibold rounded-lg hover:bg-orange-50 transition-colors text-center"
							>
