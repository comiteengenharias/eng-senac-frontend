...
export default function PortalRobotica() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepara corpo e endpoint baseado no perfil
      if (selectedProfile === 'ADMIN') {
        endpoint = `${apiBaseUrl}/api/robotica/register/admin`;
        body = {
          nomeCompleto: regFullName,
          cpf: regCpf,
          emailInstitucional: regEmail,
          codigoAutorizacao: regAuthCode,
          senha: regPassword,
          adminToken: regAuthCode,
        };
      } else if (selectedProfile === 'JUIZ') {
        endpoint = `${apiBaseUrl}/api/robotica/register/juiz`;
        body = {
          nomeCompleto: regFullName,
          cpf: regCpf,
          emailInstitucional: regEmail,
      ...
      } else if (selectedProfile === 'TECNICO') {
        endpoint = `${apiBaseUrl}/api/robotica/register/tecnico`;
        body = {
          nome: regFullName,
          cpf: regCpf,
          email: regEmail,
          instituicaoEnsino: regInstitution,
          senha: regPassword,
        };
      }
      ...
      // Tenta ler a resposta mesmo se houver erro
      let responseData = null;
      try {
        responseData = await response.json();
      } catch {}

      // Se houver dados de sucesso na resposta, considera como sucesso mesmo com erro 500
      if (!response.ok) {
        if (response.status === 500 && responseData?.userId) {
          // Erro de Entity Framework após sucesso - trata como sucesso
          await Swal.fire({
        ...
        try {
          ...
          message = errorData?.message || errorData?.detail || `Erro ${response.status}: ${errorText}`;
        } catch {
